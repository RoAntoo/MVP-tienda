  ...
  async enviarInstruccionesPago(emailCliente: string, total: Prisma.Decimal, cantidad: number): Promise<void> {
    const htmlContent = `
      <div style="font-family: monospace; color: #f0f0f0; background: #0d0d12; padding: 20px;">
        ...
        <p>¡Hola! Has iniciado la compra de <strong>${cantidad} libro(s)</strong> por un total de <strong style="color: #ff2a85;">$${total.toString()}</strong>.</p>
  ...
  async enviarLinksDescarga(emailCliente: string, productos: Producto[]): Promise<void> {
    const listaProductosHTML = productos.map(p => `
      <li style="margin-bottom: 10px;">
        ...
        <a href="${p.driveUrl}" style="color: #ff2a85; text-decoration: none;">[ DESCARGAR_ARCHIVO ]</a>
      </li>
    `).join('');
