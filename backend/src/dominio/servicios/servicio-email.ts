...
export interface ResultadoEnvioNovedad {
  aceptado: boolean;
  referencia?: string;
}
...
export interface ServicioEmail {
  ...
  enviarNovedadCatalogo(emailCliente: string, asunto: string, mensaje: string, productos: ProductoNovedad[]): Promise<ResultadoEnvioNovedad>;
  enviarNovedadPromocion(emailCliente: string, asunto: string, mensaje: string, promociones: PromocionNovedad[]): Promise<ResultadoEnvioNovedad>;
}
